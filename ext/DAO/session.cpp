...
#ifdef UNICODE
		result = sqlite3_open16(Session::m_dbFile, &m_dbHandle);
#else
		result = sqlite3_open(Session::m_dbFile, &m_dbHandle);
#endif
		if(result != SQLITE_OK)
		{
...
CString Session::m_dbFile = CString("mfct.db");

void Session::SetDB(const CString &dbFile)
{
	if(dbFile.GetLength() > 0)
	{
		Session::m_dbFile = dbFile;
	}
}
